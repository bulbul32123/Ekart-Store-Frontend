import SingleProductPage from '@/components/singlepage/SingleProductPage';
import { apiFetcher } from '@/utils/api';
import React from 'react';

export async function generateMetadata({ params: { slug } }) {
  const singleProductResponse = await apiFetcher(`/api/products?populate=*&filters[slug][$eq]=${slug}`);
  const singleProduct = singleProductResponse.data[0];
  return {
...
export default async function ProductDetails({ params: { slug } }) {
  const singleProductResponse = await apiFetcher(`/api/products?populate=*&filters[slug][$eq]=${slug}`);
  const singleProduct = singleProductResponse.data[0];
  const relatedProductsResponse = await apiFetcher(`/api/products?populate=*&[filters][categories][slug][$eq]=${singleProduct.attributes.category}&filters[slug][$ne]=${slug}`);
  const relatedProductsBrand = await apiFetcher(`/api/products?populate=*&[filters][brands][slug][$eq]=${singleProduct.attributes.brand}&filters[slug][$ne]=${slug}`);
  const relatedBrandProducts = relatedProductsBrand

  return (
    <div>
      <SingleProductPage slug={slug} products={relatedProductsResponse.data} product={singleProduct} relatedBrandProducts={relatedBrandProducts} />
    </div>
  )
}
...
//   const { cart, watchListItemExist } = useSelector((state) => state.cart);
...
//       setProducts(relatedProductsResponse.data);
...
//                   Add to Cart
//                 </button>
//                 <button onClick={() => dispatch(addProductToWatchLst({ ...product }))} disabled={watchListItemExist} className={`w-full py-4 rounded-full border border-black text-lg font-medium transition-transform active:scale-95 flex items-center justify-center gap-2 hover:opacity-75 mb-10 ${watchListItemExist && 'cursor-not-allowed opacity-20'}`}>
//                   Wishlist
